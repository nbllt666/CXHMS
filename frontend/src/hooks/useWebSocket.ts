...
export function useWebSocket(options: WebSocketOptions): UseWebSocketReturn {
  ...
  const connect = useCallback(() => {
    ...
    wsRef.current = ws
  }, [agentId, timeout, onMessage, onAlarm, onError, onConnect, onDisconnect, startPingInterval, clearPingInterval])
  ...
  const sendMessage = useCallback((message: string, images?: string[]) => {
    if (wsRef.current?.readyState !== WebSocket.OPEN) {
