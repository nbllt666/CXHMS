...
from .models import (
    BackupInfo, BackupManifest, BackupType, BackupStatus,
    RestoreResult, BackupStats
)
...
class BackupManager:
    ...
    async def restore_backup(
        ...
        if backup.status != BackupStatus.COMPLETED:
            return RestoreResult(
                success=False,
                error_message=f"备份状态不正确: {backup.status.value}"
            )
    ...
    def get_stats(self) -> BackupStats:
        ...
        stats.full_backups = sum(1 for b in self._backups.values() if b.backup_type == BackupType.FULL)
        stats.incremental_backups = sum(1 for b in self._backups.values() if b.backup_type == BackupType.INCREMENTAL)
