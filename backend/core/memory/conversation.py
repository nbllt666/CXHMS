...
class MemoryConversationEngine:
    ...
    async def _parse_command(
        ...
        return MemoryCommand(
            ...
            parameters={"original_message": user_message},
            confidence=0.0
        )
    ...
    def _extract_parameters(self, message: str, command_type: str) -> Dict[str, Any]:
        ...
        if command_type == "search":
            # 提取搜索关键词
            # 移除命令词
            keywords = re.sub(r"搜索|查找|找一下|有没有|查询|search|find|look for", "", message, flags=re.IGNORECASE).strip()
    ...
    async def _handle_delete(
        ...
        try:
            ...
            if success:
                return {
                    "status": "success",
                    "message": f"记忆 ID {memory_id} 已删除"
                }
            else:
                return {
                    "status": "error",
                    "message": f"删除失败，记忆可能不存在"
                }
...
回复："""
            ...
            return {
                "status": "success",
                "message": text if text else "抱歉，我没有理解您的指令。请尝试使用 '帮助' 查看可用命令。"
            }
