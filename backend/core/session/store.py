...
from .models import Session, SessionMessage, SessionType, SessionStats
...
class SessionStore:
    ...
    def create_session(
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute('''
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                session.id, session.workspace_id, session.title, session.user_id,
                session.session_type.value, session.created_at.isoformat(),
                session.updated_at.isoformat(), session.last_accessed_at.isoformat(),
    ...
    def add_message(
        ...
        with self._get_connection() as conn:
            ...
            ''', (
                ...
                message.content_type, json.dumps(message.metadata, ensure_ascii=False),
                message.tokens, message.created_at.isoformat(), message.is_deleted
            ))
    ...
    def get_statistics(self, workspace_id: Optional[str] = None) -> SessionStats:
        ...
        with self._get_connection() as conn:
            ...
            # 激活会话数
            cursor.execute(
                f"SELECT COUNT(*) FROM sessions {base_where} AND is_active = TRUE",
                params
            )
