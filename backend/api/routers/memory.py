...
async def recall_memory(memory_id: int, emotion_intensity: float = 0.0, agent_id: str = "default"):
    ...
    try:
        memory_mgr = get_memory_manager()
        # B4: 透传 agent_id，按 agent 表查询，避免跨 agent 记忆串扰
...
async def execute_secondary_command(
    ...
    from backend.dependencies import get_memory_manager, get_secondary_router
...
async def get_secondary_commands():
    from backend.dependencies import get_secondary_router
