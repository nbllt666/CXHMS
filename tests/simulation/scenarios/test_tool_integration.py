...
def test_write_long_term_memory_tool_executed(sim_app):
    ...
    # 副作用验证：通过 ServiceState 中的 MemoryManager 直接检索（避免再走 HTTP 增加噪声）
    import backend.dependencies as _deps

    mm = _deps._service_state.memory_manager
    assert mm is not None, "MemoryManager 未注入 ServiceState（lifespan 未启动？）"
...
@pytest.mark.skip(
    reason=(
        "save_memory 工具未注册：backend/core/tools/memory_tools.py 实际为完整文件（3870 字节），"
        "但 app.py 模拟模式 lifespan 中并未调用 register_memory_tools，"
        "因此 save_memory 实际未注册到 tool_registry。待在 app.py lifespan 中"
        "添加 register_memory_tools 调用后启用本测试。"
