...
class Gemma4ReasoningParser(BaseThinkingReasoningParser):
    ...
    def is_reasoning_end_streaming(
        ...
        Root cause: BaseThinkingReasoningParser.is_reasoning_end_streaming
        only checks for <channel|> end token. When the model emits <|tool_call>
        directly (without preceding reasoning), reasoning_ended is never set
        to True, so extract_tool_calls_streaming is never called, and
        <|tool_call> leaks as content text.
        """
